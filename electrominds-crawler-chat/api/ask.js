...
// CAMBIA este número por tu WhatsApp real (código de país +57)
const WHATSAPP = "https://wa.me/573001234567?text=Hola%20Electrominds,%20necesito%20asesor%C3%ADa";

export default async function handler(req, res) {
  const { q } = req.method === "POST" ? (req.body || {}) : (req.query || {});
  if (!q || !String(q).trim()) return res.status(400).json({ ok: false, error: "Falta parámetro q" });

  const idx = getIndex();
  if (!idx.docs.length) {
    await (await import("./reindex.js")).default({ method: "GET" }, { status: () => ({ json: () => {} }) });
  }
